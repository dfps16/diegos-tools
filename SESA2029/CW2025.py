...
def zero_lift(m,p,n):
    ...
    for i in range(n):
        ...
        if x<p:
            intnext=2.0*m/p**2*(p-x)*(1.0-np.cos(theta)) # i.e. (dz/dx)*(1-cos(theta))
        else:
            intnext=2.0*m/(1.0-p)**2*(p-x)*(1.0-np.cos(theta))    
        sum0=sum0+0.5*dtheta*(intlast+intnext)/np.pi # trapezoid integration
        intlast=intnext
...
def lifting_line(alpha,AR,tr,a0,alpha_zero,n):
    ...
    coeffs=np.zeros(n)
    for i in range(n):
        theta=np.pi/2.*np.real(2*(i+1)-1)/np.real(2*n)  # collocation points for theta
        c=(2.*(1.+(tr-1.)*np.cos(theta)))/(1.+tr)       # chord for tapered wing
        rhs[i]=alpha-alpha_zero                         # set rhs vector
        for j in range(n):
            k=2*(j+1)-1
            A[i,j]=np.sin(k*theta)*(4.*AR/(a0*c)+k/np.sin(theta))  # set matrix elements
...
if len(ID)==8:
    ...
    nthinaerofoil=64 # number of points to use for thin aerofoil evaluation of zero lift angle    
    alpha_zero_lift=zero_lift(np.real(NACA1)/100.0,np.real(NACA2)/10.0,nthinaerofoil)
    ...
    AR=2.0*float(ARhalf)
    dalpha=0.01
    (CL0,CDi,delta)=lifting_line(0.0,AR,float(TR),a0,alpha_zero_lift*np.pi/180.0,n_lift_line)
    (CL1,CDi,delta)=lifting_line(dalpha,AR,float(TR),a0,alpha_zero_lift*np.pi/180.0,n_lift_line)
    a=(CL1-CL0)/dalpha
    ...
    for ialpha in range(9):
        ...
        (CL,CDi,delta)=lifting_line(alpha,AR,float(TR),a0,alpha_zero_lift*np.pi/180.0,n_lift_line)
        #print(round(alpha*180/np.pi,2),'      ',round(CL,4),'  ',round(CDi,5))
        print("{:5.2f}".format(alpha*180/np.pi),"{:11.4f}".format(CL),"{:10.5f}".format(CDi))
